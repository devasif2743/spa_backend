...
class AppointmentController extends Controller
{
    ...
    {
        $req->validate(['date' => 'required|string']);

        // 1) Extract date part ONLY (avoid timezone shifts)
        $dateStr  = $req->query('date');               // e.g. "2025-09-19 17:52:56" or "2025-09-19"
        $day      = Str::substr($dateStr, 0, 10);      // "2025-09-19"
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $day)) {
            return response()->json(['message' => 'Invalid date'], 422);
        }

        // If you want to anchor to IST explicitly, uncomment the next 2 lines.
        // $tz       = 'Asia/Kolkata';
        // $dayStart = Carbon::createFromFormat('Y-m-d H:i:s', "$day 00:00:00", $tz)->startOfDay()->toDateTimeString();

        // 2) Naive local (no-TZ) day window prevents off-by-one if DB stores local timestamps
        ...
        $therapists = User::where('role', 'therapist')
            ->orderBy('name')
            ->get(['id', 'name', 'shiftStart', 'shiftEnd']);

            
        //dd($therapists->pluck('id'));

        // 4) Appointments that OVERLAP the day (not just start on the day)
        $apps = appointment::whereIn('staff_id', $therapists->pluck('id'))
            ->where(function ($q) use ($dayStart, $dayEnd) {
                $q->whereBetween('start_time', [$dayStart, $dayEnd])              // start inside day
                    ->orWhereBetween('end_time',   [$dayStart, $dayEnd])            // end inside day
                    ->orWhere(function ($qq) use ($dayStart, $dayEnd) {             // spans entire day
                        $qq->where('start_time', '<', $dayStart)
                            ->where('end_time',   '>', $dayEnd);
                    });
            })
            ->orderBy('start_time')
            ->get(['id', 'staff_id', 'start_time', 'end_time', 'customer_source']);

        $byTherapist = $apps->groupBy('staff_id');



        $out = $therapists->map(function ($t) use ($byTherapist, $day) {
            $ws = $t->shiftStart ?: '09:00:00';
            $we = $t->shiftEnd   ?: '20:00:00';
            $workStart = Carbon::parse("$day $ws");
            $workEnd   = Carbon::parse("$day $we");

            $items = ($byTherapist[$t->id] ?? collect())->map(function ($a) {
                $s = Carbon::parse($a->start_time);
                $e = Carbon::parse($a->end_time);
                $src = strtolower($a->customer_source ?? '');
                $label = ($src === 'walkin' || $src === 'walk-in') ? 'Walk-in'
                    : (str_contains($src, 'online') ? 'Booked online' : 'Service');

                return [
                    'start' => $s->toDateTimeString(),
                    'end'   => $e->toDateTimeString(),
                    'chip'  => $label . ' • ' . $s->format('g:i A') . ' - ' . $e->format('g:i A'),
                ];
            })->values();
            ...
            return [
                ...
                'working'   => [
                    'start' => $workStart->toDateTimeString(),
                    'end'   => $workEnd->toDateTimeString(),
                    'label' => 'Working: ' . $workStart->format('g:i A') . ' - ' . $workEnd->format('g:i A'),
                ],
                'booked_title'     => 'Booked',
                'booked'           => $items,
                'suggested_starts' => [],
    ...
    private function labelFromSource($src): string
    {
        if (!$src) return 'Service';
        $s = strtolower($src);
        if (str_contains($s, 'online')) return 'Booked online';
        if (str_contains($s, 'walk'))   return 'Walk-in';
        return ucfirst($src);
    }
